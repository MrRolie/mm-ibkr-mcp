...
4. Telegram approval requests (`request_trade_approval`, `request_trade_intent_approval`, `request_environment_change`) block synchronously. Do not manually poll for their completion; simply wait for the tool to return the final approved, denied, or expired status.
