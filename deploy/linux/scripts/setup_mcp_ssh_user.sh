...
Options:
  ...
  --repo-dir PATH           mm-ibkr-gateway repo root (default: ${REPO_DIR_DEFAULT})
...
print(cfg.watchdog_log_dir)
PY
    )

    if [ "${#RUNTIME_PATHS[@]}" -lt 4 ]; then
        echo "Error: failed to load runtime config paths." >&2
        exit 1
    fi
    ...
    WATCHDOG_LOG_DIR="${RUNTIME_PATHS[3]}"
}
...
prepare_runtime_env() {
    local venv_path="${USER_HOME}/.local/share/mm-ibkr-gateway/.venv"

    run install -d -o "${SSH_USER}" -g "${SSH_USER}" -m 0755 "${USER_HOME}/.local/share/mm-ibkr-gateway"
    ...
    run chown -R "${SSH_USER}:${SSH_USER}" \
        "${USER_HOME}/.local/share/mm-ibkr-gateway" \
        "${USER_HOME}/.cache/uv"
}
